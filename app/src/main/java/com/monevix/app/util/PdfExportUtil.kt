...
object PdfExportUtil {
    ...
    fun exportSettlement(context: Context, settlement: SettlementEntity, items: List<SettlementItemEntity>): Uri {
        ...
        y = drawSubtitle(canvas, settlement.companyName.ifBlank { "Monevix" }, y)
        y += 16f
    ...
    fun exportMonthlySummary(
        ...
        y = drawSubtitle(canvas, companyName.ifBlank { "Monevix" }, y)
    ...
    fun exportPayment(context: Context, payment: PaymentEntity, items: List<PaymentItemEntity>): Uri {
        val document = PdfDocument()
        var pageNumber = 1
        ...
        y = drawTitle(canvas, "Comprobante de pago", y)
        y = drawSubtitle(canvas, payment.companyName.ifBlank { "Monevix" }, y)
        y += 10f
        y = drawLine(canvas, "Trabajador: ${payment.workerName}", y, bold = true)
        y = drawLine(canvas, "Periodo: ${payment.periodLabel}", y)
        y = drawLine(canvas, "Fecha de pago: ${dateFormat.format(Date(payment.paidAtMillis))}", y)
        y += 8f
        y = drawDivider(canvas, y)
        y += 10f
        y = drawLine(canvas, "Productividad registrada", y, bold = true)
        ...
        items.forEach { item ->
            ...
            y = drawLine(canvas, "${SimpleDateFormat("dd/MM/yyyy", Locale("es", "CO")).format(Date(item.dateMillis))} · ${item.code} - ${item.name}", y)
            y = drawLine(canvas, "  ${item.quantity} x ${CurrencyFormatter.format(item.unitPrice)} = ${CurrencyFormatter.format(item.subtotal)}", y)
        }
        y += 8f
        y = drawDivider(canvas, y)
        y += 16f
        drawLine(canvas, "TOTAL PAGADO: ${CurrencyFormatter.format(payment.total)}", y, bold = true, size = 16f)

        document.finishPage(page)
        val fileName = "pago_${payment.workerName.sanitize()}_${payment.id}.pdf"
        return document.saveAndGetUri(context, fileName)
    }
