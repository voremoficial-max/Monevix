                ...
                if (!isSaving) {
                    ...
                    scope.launch {
                        when (val result = viewModel.saveSettlement(
                            workerId,
                            worker?.name ?: "",
                            context.getSharedPreferences("monevix_preferences", android.content.Context.MODE_PRIVATE).getString("company_name", "").orEmpty(),
                            context.getSharedPreferences("monevix_preferences", android.content.Context.MODE_PRIVATE).getString("payment_frequency", "WEEKLY").orEmpty()
                        )) {
